...
class ReSendNotificationEmail extends Job implements ShouldQueue
{
    ...
    */
	...
	private function sendAPI(){
	    $input = $this->input;
        $input['paramMail'] = json_decode($this->input['push_notification_param'],1);
        ...
        switch($input['api_type']){
            case 'Sale':
                $paramSale['saleMan'] = strtolower($input['paramMail']['saleMan']);
                ...
                break;
            case 'Tech':
                $paramTech['team'] = strtolower($input['paramMail']['team']);
        ...
        $param['api_send_count'] = (int)$input['api_send_count'] + 1;
