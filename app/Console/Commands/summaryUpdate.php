...
//    protected $signature = 'summaryData {fromDay} {numDays}';
    protected $signature = 'summaryData:Update';
     ...
     * @var string
     */
    protected $description = 'update summary';
...
//        {
        DB::beginTransaction();
        try {
            $day_update_summary=Redis::exists('day_update_summary');
              $dayToRemove=Redis::lpop('day_update_summary');
            if ($day_update_summary) {
...
//                    $this->getAllSummaryData($dayToRemove);
                $this->SummaryAllInOne($dayToRemove);
                echo 'fixed-' . $dayToRemove . "\n";
            } else {
                $dayToRemove='0';
                echo 'Khong co ngay can update \n';
            }
            DB::commit();
        } catch (Exception $ex) {
               Redis::lpush('day_update_summary',$dayToRemove);
            DB::rollback();
        }
    }

    public function SummaryAllInOne($timeBegin) {
        DB::beginTransaction();
        try {
