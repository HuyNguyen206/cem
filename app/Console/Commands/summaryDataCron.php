...
class summaryDataCron extends Command {
...
//    protected $signature = 'summaryData {fromDay} {numDays}';
    protected $signature = 'summaryDataCron';
    ...
    public function handle() {
        set_time_limit(0);
        //Tổng hợp lại dữ liệu đã update trong ngày
        while((Redis::get('day_update_summary')) != null && !empty(json_decode(Redis::get('day_update_summary'))))
        {
                DB::beginTransaction();
                try {
                    $day_update_summary=json_decode(Redis::get('day_update_summary'));
                    $dayToRemove=array_pop($day_update_summary);
                     Redis::set('day_update_summary', json_encode($day_update_summary));                  
                    $summaryTime = new SummaryTime();                   
                    $time_id = $summaryTime->getTimeIdByDay($dayToRemove);
                    DB::table('summary_action')->where('time_id', '=', $time_id)->delete();
                    DB::table('summary_csat')->where('time_id', '=', $time_id)->delete();
                    DB::table('summary_nps')->where('time_id', '=', $time_id)->delete();
                    DB::table('summary_opinion')->where('time_id', '=', $time_id)->delete();
                    DB::table('summary_reason')->where('time_id', '=', $time_id)->delete();
                    $this->getAllSummaryData($dayToRemove);
                    DB::commit();
                     echo 'fixed-' . $dayToRemove . "\n";
                } catch (Exception $ex) {
...
//        }
        //Tổng hợp dữ liệu tất cả các bảng summary trong ngày
        $fromDay = date('y-m-d');
        DB::beginTransaction();
        try {            
            ...
            DB::table('queue')->insert(
                    ['input' => $fromDay, 'type' => 'summary', 'created_at' => date('y-m-d'), 'output' => '']
            );
            DB::rollback();
        }
//        }
