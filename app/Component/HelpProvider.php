...
class HelpProvider {
	...
	public static function validateDate(&$date){
			...
			if($testDate === false){return false;}
			return true;
	}
	...
	public static function validateDateStartEndForSearchFullDay(&$date_start, &$date_end){
		...
		if($testStart === false){return false;}
		$testEnd = date_create($date_end);
		if($testEnd === false){return false;}
	...
	public function getCondition($name) {
        $condition = [
            ...
            'supportMD5ForISC' => ['ContractID','TransactionID'],
            'generateLinkSurvey' => ['ContractID','TransactionID','Type'],
            'getInfoSurveyByContractNumber' => ['ContractNumber'],
        ];
        ...
        switch($type){
            ...
            case 'Sale':
                Mail::send('emails.sendNotification', ['param' => $input['paramMail']], function ($message) use ( $mail, $cc, $subject) {
                    $message->from('customer-voice@opennet.com.kh', 'Support');
                    $message->to($mail);
                    $message->cc($cc);
