...
<div class="page-content">
    ...
    <div class="row">
        <div class="col-xs-12">
            ...
            <form class="form-horizontal" role="form" method="POST" action="{{ url(main_prefix.'/update-role') }}">
                {!! csrf_field() !!}
                    ...
                    <div class="col-sm-9">
                        ...
                        <div id="oldSelectGroup" class="col-xs-12 col-sm-5 no-padding">
                            ...
                            @else
                                <select name="group1[]" multiple="" class="col-xs-12 no-padding-left chosen-select" id="form-field-select-4" data-placeholder="{{empty($group)? trans($transFile.'.NoGroupExist') : trans($transFile.'.PleaseChooseOneGroup')}}">
                                    @foreach($group as $val)
                                        @if(array_search($val->name, $editRoles['group']) !== false)
                                            <option selected="selected" value="{{ $val->id }}">{{$val->display_name }}</option>
                                        @else
                                            <option value="{{ $val->id }}">{{$val->display_name }}</option>
                                        @endif
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-9">
                        @if(empty(old('name')))
                            <input type="text" class="col-xs-12 col-sm-5" name="name" value="{{ $editRoles['roleDisplayName'] }}" placeholder="{{trans($transFile.'.Input here')}}" oninvalid="this.setCustomValidity('{{trans($transFile.'.require name')}}')" oninput="setCustomValidity('')">
                        @else
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-9">
                        @if(empty(old('description')))
                            <input type="text" class="col-xs-12 col-sm-5" name="description" value="{{$editRoles['roleDescription']}}" placeholder="{{trans($transFile.'.Input here')}}">
                        @else
                        ...
                        <div class="col-xs-12 col-sm-5 no-padding">
                            @if(empty(old('level')))
                                <select name='level' class="col-xs-12 no-padding-left" id="form-field-select-1">
                                    @foreach($role as $val)
                                        @if($editRoles['roleLevel'] == $val->level + 1)
