...
<div class="page-content">
    <?php
    $controller = 'csatstaff';
    $transfile = 'report';
    $common = 'common';
    $prefix = main_prefix;
    ?>
    <!-- PAGE CONTENT BEGINS -->
    <form class="form-horizontal" role="form" method="POST" action="{{ url('/'.$prefix.'/'.$controller.'/general-report') }}">
        {!! csrf_field() !!}
            ...
            <div class="col-xs-12">
                <div class="row">
                    ...
                    <div class="btn-group" style="vertical-align: top!important;">
                        <button id="reportrange" type="button" class="btn btn-danger" style="height: 42px;margin-right: 3px;">Chọn ngày</button>
                ...
                <div id='container_info_report'>
                    include('report.csatReport',['survey' => $survey, 'total' => $total, 'avg' => $avg, 'region' => $region, 'from_date' => $from_date, 'to_date' => $to_date])
                </div>
            </div>
        </div>
...
//   var typeExport=
    ...
    $('#sel_region').multiselect({
        ...
        onChange: function (element, checked) {
            ...
            var cat_id = t.join(',');
            $.ajax({
                url: '<?php echo url('/' . $prefix . '/location/getLocationByRegion') ?>',
                cache: false,
                type: "POST",
                dataType: "json",
    ...
    function getDataToChartAfterChoose() {
        ...
        $.ajax({
            url: '<?php echo url('/' . $prefix . '/' . $controller . '/general-report') ?>',
            cache: false,
            type: "POST",
            dataType: "html",
