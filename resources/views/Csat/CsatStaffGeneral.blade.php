...
<div class="page-content">
    ...
    ?>
    <!-- PAGE CONTENT BEGINS -->
    <form class="form-horizontal" role="form" method="POST" action="{{ url('/'.$prefix.'/'.$controller.'/csat-staff/general') }}">
        {!! csrf_field() !!}
        ...
        <div class="row">
            <div class="col-xs-12">
                ...
                <div id='container_info_report'>
                    @include('Csat.CsatStaffGeneralPart1')
                </div>
            </div>
        </div>
...
//   var typeExport=
    ...
    function getDataToChartAfterChoose() {
        ...
        $.ajax({
            url: '<?php echo url('/' . $prefix  . '/csat-staff/general') ?>',
            cache: false,
            type: "POST",
            dataType: "html",
    ...
    $("#exportToExcel").click(function () {
        $.redirect('<?php echo url('/' . $prefix . '/csat-staff/exportGeneral') ?>', {_token: $('input[name=_token]').val(), type: $("#typeReport").val(), region: $('#reg_hidden').val(), from_date: $('#from_date_hidden').val(), to_date: $('#to_date_hidden').val(), branch: $('#state_hidden').val(), branchcode: $('#branchcode_hidden').val(), viewStatus: $('#viewStatusHidden').val()});
    });
